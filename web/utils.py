"""Utility functions for loading and processing prediction data."""
...
def find_league_predictions() -> dict:
    ...
    Returns:
        dict: Dictionary with league names as keys and file info (path and match week) as values.
        Example:
        {
            'Premier-League': {
                'file': Path('...Premier-League_week_10_predictions.csv'),
                'match_week': 10
            },
            ...
        }
    ...
    for league in leagues:
        # Find files matching the league pattern
        pattern = f"{league}_week_*_predictions.csv"
        matching_files = list(tables_dir.glob(pattern))

        if matching_files:
            # Get the file with the latest match week
            latest_file = None
            latest_week = -1

            for file in matching_files:
                # Extract match week number
                try:
...
def get_predictions() -> dict:
    ...
    Returns:
        dict: Dictionary with league names as keys and data (table HTML and match week) as values.
        Example:
        {
            'Premier-League': {
